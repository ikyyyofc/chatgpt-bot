...
export function updateParticipantStore(groupJid, participants, sock = null) {
    ...
    for (const participant of participants) {
        ...
        // coba ambil nama dari sock.getName
        let pushName = existing?.pushName || null;
        
        if (!pushName && sock) {
            try {
                const contactName = sock.getName(participant.jid);
                if (contactName && contactName !== participant.jid.split('@')[0]) {
                    pushName = contactName;
                    console.log(`   📝 Got name from contact: ${participant.jid.split('@')[0]} -> "${pushName}"`);
                }
            } catch (e) {}
        }
