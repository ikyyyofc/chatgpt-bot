...
export function getParticipantName(jid, groupJid) {
    try {
        ...
        if (stored) {
            // prioritas: pushName > notify > formatted number
            if (stored.pushName) return stored.pushName;
...
export function cleanTextFromMentions(text, mentionedJid, botJid, botNumber, botLid, groupJid) {
    ...
    for (const mentionJid of mentionedJid) {
        ...
        // extract nomor dari mention (bisa format LID atau JID)
        // contoh: "161199232659495@lid" -> "161199232659495"
        // contoh: "6287738172692@s.whatsapp.net" -> "6287738172692"
        const mentionNumber = mentionJid.split('@')[0].split(':')[0];
        
        // replace @nomor dengan nama
        // pake lookahead biar ga replace angka yang kebetulan sama di tengah text
