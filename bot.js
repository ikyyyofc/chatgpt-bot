...
const connect = async () => {
    ...
    setTimeout(async () => {
      try {
        const code = await sock.requestPairingCode(config.PAIRING_NUMBER, 'IKYYCHAT')
    ...
    switch (statusCode) {
        case 408:
            console.error(colors.yellow('🔄 Connection timed out. Reconnecting...'));
            await connect();
            break;
        case 503:
            console.error(colors.yellow('🔄 Service unavailable. Reconnecting...'));
            await connect();
            break;
        case 428:
        case 515:
            console.error(colors.yellow('🔄 Connection closed. Reconnecting...'));
            await connect();
            break;
        case 403:
